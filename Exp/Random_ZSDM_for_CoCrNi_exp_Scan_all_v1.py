...
def SRO_extract(i_threshold):
    ...
    np.savetxt('pre_extract\\'+ 'Random_'+element_name_chosen+element_name_chosen +
...
for element_name_chosen in (element_1_name, element_2_name, element_3_name):
    ...
    data_predictions = np.load('random_CoCrNi_test_data_'+element_name_chosen+element_name_chosen+'_%s_%s_%s_%s_%s_%s.npy'%(int(data_min['c']), int(data_max['c']), int(data_max['b']), int(data_max['a']), voxel, stride))
    ...
    np.save('random_pixel_probability_flatten_'+element_name_chosen+element_name_chosen, pixel_probability_flatten)
